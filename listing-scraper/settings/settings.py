...
class Settings:
    ...
    @staticmethod
    def __init_max_listings_per_chunk(settings: dict) -> int:
        """
        Initialize the max_listings_per_chunk from the settings dictionary.
        ...
        max_listings_per_chunk = settings.get("max_listings_per_chunk")

        if not isinstance(max_listings_per_chunk, int):
            logger.warning("Max_listings_per_chunk is not of int type. Max_listings_per_chunk is set to default")
            max_listings_per_chunk = Constans.DEFAULT_MAX_LISTING_PER_CHUNK
        if max_listings_per_chunk < 0 or max_listings_per_chunk < 0:
